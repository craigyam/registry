...
func TestNewExternalCatalogNotRedis(t *testing.T) {
	...
	conf.store = "test"
	catalog, err := newExternalCatalog(conf, "test")
...
func TestNewExternalCatalogNilConfig(t *testing.T) {
	catalog, err := newExternalCatalog(nil, "test")
...
func TestRedisRegisterInstanceWithCatalogTTL(t *testing.T) {
	...
	conf := createNewRedisConfig(2 * DefaultConfig.DefaultTTL)

	conn.Clear()

	catalog := createRedisCatalog(conf)
	...
	//s, instance := generateMockHScanCommandOutput("inst-id", si)
	//cmd := conn.GenericCommand("HSCAN").Expect(s)
...
func TestExternalRenewInstancesConcurrently(t *testing.T) {
	...
	const ttl = (10) * time.Second //defaultTTL //testMediumTTL
